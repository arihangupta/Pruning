...
Enhanced with single-image energy tracking after global finetuning.
...
# -------------------------
# NEW: Single-image prediction helper with energy tracking
# -------------------------
def predict_single_image_with_energy(model: nn.Module, image: np.ndarray, img_size=IMG_SIZE, 
                                    tracker_save_dir=None, project_name="single_image"):
    """
    Run a single image through the model and track energy with CodeCarbon.
    Returns predicted class index, softmax probabilities, and energy metrics.
    """
    tfms = T.Compose([
        T.ToPILImage(),
        T.Resize((img_size, img_size)),
    ...
    if CODECARBON_AVAILABLE and tracker_save_dir is not None:
        tracker = start_tracker(tracker_save_dir, project_name, measure_power_secs=0.1)
...
# Original predict_single_image function (for compatibility)
...
def read_latest_tracker_row(save_dir: str, project_name: str):
    """
...
def stop_tracker_and_get_metrics(tracker, save_dir: str, project_name: str):
    ...
    raw = read_latest_tracker_row(save_dir, project_name)
    ...
    try:
        ...
        for method in METHODS:
            for target_ratio in TARGET_RATIOS:
                ...
                # NEW: Single-image prediction with energy tracking for pruned model AFTER global finetuning
                print(f"=== MEASURING SINGLE-IMAGE ENERGY FOR PRUNED MODEL (after global finetuning) ===")
                pruned_pred, pruned_probs, pruned_energy = predict_single_image_with_energy(
                    current_model, sample_raw, tracker_save_dir=SAVE_DIR, 
                    project_name=f"{dataset_name}_{method}_r{int(target_ratio*100)}_single_final"
                )
                
                # Add single-image results to CSV
                pruned_single_row = row.copy()
                pruned_single_row["Stage"] = "pruned_single_after_global_finetune"
                ...
                print(f"Pruned single-image pred: {pruned_pred} (true: {sample_label})")
                print(f"Single-image energy: {pruned_single_row['SingleEnergy_kWh']:.8f} kWh")
                print(f"Single-image emissions: {pruned_single_row['SingleEmissions_kg']:.8f} kg CO2eq")
                
                # Compare with baseline single-image energy
                if not math.isnan(baseline_single_row["SingleEnergy_kWh"]) and not math.isnan(pruned_single_row["SingleEnergy_kWh"]):
                    energy_reduction = ((baseline_single_row["SingleEnergy_kWh"] - pruned_single_row["SingleEnergy_kWh"]) / baseline_single_row["SingleEnergy_kWh"]) * 100
                    print(f"Single-image energy reduction: {energy_reduction:.2f}%")
