#!/usr/bin/env python3
"""
Progressive PGTO pruning for Dermamnist (fixed surgery + progressive updates + KD).

- Progressive: importance computed on the updated model each stage
- Surgery fixes: grouped expansion channels preserved; prev_out_idx uses contiguous groups
- KD: applied after progressive prune+local calibrations, before final global finetune
- Metrics saved per key checkpoint
"""
...
SAVE_DIR   = "/home/arihangupta/Pruning/dinov2/Pruning/saved_models_pgto_fixed"
...
KD_ALPHA = 0.7
KD_TEMPERATURE = 3.0
KD_MAX_BATCHES = None  # set to int to limit batches per epoch
...
def build_pruned_resnet_and_copy_weights_fixed(base_model: nn.Module, keep_indices: Dict[str, np.ndarray], num_classes: int):
    ...
    # But conv1 of Bottleneck expects previous stage expanded? For layer1 conv1 input is base_model.conv1.out_channels
    # We'll track prev_out_idx per-block inside stage loop.
    ...
    for stage_name in ["layer1","layer2","layer3","layer4"]:
        ...
        # For the *first block in the stage*, the in_idx is prev_out_idx (from previous stage).
        # After each block, prev_out_idx becomes the expanded output channels for that block (grouped)
        # For copying within a stage: conv1 output channels correspond to the kept_planes (one channel per plane),
        # conv2 input/output use plane-count, conv3 expands to plane*expansion.
        for block_idx, (old_block, new_block) in enumerate(zip(old_stage, new_stage)):
            # in indices into old block's expected input channels:
            ...
            # ----- conv1: old_block.conv1: shape [planes, in_channels, 1, 1]
            # copy rows corresponding to kept plane indices, and columns corresponding to in_idx
            # If the old_block.conv1.weight shape is [old_planes, old_in_channels, 1,1]
            # we want new_block.conv1.weight = old_block.conv1.weight[out_planes][:, in_idx, :, :]
            # BUT old_block.conv1 expects input channels matching prev stage expanded indices length
            # Ensure indexing safety
            old_w = old_block.conv1.weight.data
            # old_w shape: [old_planes, old_in_channels, k, k] (k=1)
            ...
            # bn1 params: per-plane (non-expanded)
            ...
            # ----- conv2: old_block.conv2: shape [planes, planes, k, k]
            ...
            # Old conv3 has shape [old_planes*exp, old_planes, k, k]
            # We must pick expanded groups corresponding to kept planes.
            expanded_rows = []
            for p in out_planes.cpu().numpy().tolist():
                idx0 = p * expansion
                idx1 = (p + 1) * expansion
                expanded_rows.append(torch.arange(idx0, idx1, dtype=torch.long, device=DEVICE))
            expanded_rows = torch.cat(expanded_rows)  # indices of rows to KEEP (in old conv3 out_channels)
            # columns correspond to old conv2 outputs -> we pick out_planes
            ...
            # update prev_out_idx for next block: prev_out_idx must be the expanded output channels indices
            # in the *original base_model* numbering (so next block's in_idx slices the correct columns)
            prev_out_idx = expanded_rows

        # end blocks in stage

        # For the next stage, we need prev_out_idx to reflect the output channels of the last block in this stage
        # which we already set to expanded_rows for the last block.

    # Copy final fc weights: fc expects input size = last_stage_kept * expansion
    ...
    # base_model.fc.weight shape: [num_classes, old_fc_in]
...
# Global finetune
# -------------------------
def global_finetune(model, train_loader, epochs=FINAL_FINETUNE_EPOCHS, lr=FINAL_LR):
    model.train()
...
for method in METHODS:
    for target_ratio in TARGET_RATIOS:
        ...
        for s in STAGES:
            ...
            # Explanation: we only changed stage s now; for other stages we keep their current width (orig) for surgery.
...
# Save CSV
csv_path = os.path.join(SAVE_DIR, "dermamnist_pgto_pruning_metrics_progressive_fixed.csv")
