...
METHODS = ["quantization", "hybrid_pruning", "hybrid_pruning_fp16", "slim_kd", "slim_kd_fp16", "regional_pruning", "regional_pruning_fp16"]
        ...
        for method in METHODS:
            if method == "quantization":
                ...
                for compress_ratio in TARGET_COMPRESS_RATIOS:
                    ...
                    if "hybrid_pruning_fp16" in METHODS:
                        fp16_method = "hybrid_pruning_fp16"
                        print(f"\n=== {fp16_method.upper()} (FP16 of hybrid_pruning) ===")
            ...
            elif method == "regional_pruning":
                print(f"\n=== REGIONAL PRUNING (FP32 pruned, no KD) ===")
                for compress_ratio in TARGET_COMPRESS_RATIOS:
                    keep_ratio = 1 - compress_ratio
                    print(f"  Compression: {compress_ratio*100}% (keep ratio {keep_ratio})")
                    ...
                    if "regional_pruning_fp16" in METHODS:
                        fp16_method = "regional_pruning_fp16"
                        print(f"\n=== {fp16_method.upper()} (FP16 of regional_pruning) ===")
                        conversion_proj = f"{dataset_name}_{fp16_method}_r{int(compress_ratio*100)}compressed_conversion"
                        fp16_model, conversion_energy_kwh, conversion_emissions_kg = create_amp_quantized_version(
                            current_model, SAVE_DIR, conversion_proj
                        )
